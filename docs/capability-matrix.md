...
| 多周期预测共振 | implemented_verified | `research/resonance.py`, MCP `predict_resonance`, UI 🎯 | 1d/5d/20d 三周期方向一致性：全同向=强共振(100%)/两周期=共振(70%)/分歧=观望；**强共振覆盖决策树单周期方向** | 结构/强趋势/无数据/共振覆盖 4项 | 确定性模式控成本 |
